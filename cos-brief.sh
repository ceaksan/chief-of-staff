#!/bin/bash
# Chief of Staff - Unified CLI
# Usage: cos [command] [args]
...
run_brief() {
    ...
    if [ -z "$NOTE" ]; then
        echo "Daily note bos. Once pipeline calistir: cos run"
        exit 1
    fi
...
# Lock check (skip for read-only commands)
acquire_lock() {
...
run_status() {
    ...
    if [ ! -f cos.db ]; then
        echo "No database found. Run 'cos run' to start."
        exit 0
    fi
    python -c "
import sqlite3, json
